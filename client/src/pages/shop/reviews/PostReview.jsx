...
const PostReview = ({ isModalOpen, handleCloseReviewModal }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            alert("Something went wrong: " + (error.data?.message || error.message || "Unknown error"));
        }
    ...
    return (
        <div className={`fixed inset-0 bg-black/90 flex items-center justify-center z-40 px-2 ${isModalOpen ? 'block' : 'hidden'}`}>
            <div className='bg-white p-6 rounded-md shadow-lg w-96 z-50'>
                ...
                <textarea value={comment} onChange={(e) => setComment(e.target.value)} rows="4" className='w-full border border-r-gray-300 rounded-md mb-4 p-4 focus:outline-none'></textarea>

                <div className='flex justify-end gap-2'>
                    <button onClick={handleCloseReviewModal} className='px-4 py-2 bg-gray-300 rounded-md'>Cancel</button>
