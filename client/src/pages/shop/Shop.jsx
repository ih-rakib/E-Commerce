...
const Shop = () => {
    ...
    const endingProduct = Math.min(startingProduct + products.length - 1, totalProducts);
        ...
        <>
            ...
            <section className="section__container">
                <div className="flex flex-col md:flex-row md:gap-16 gap-8">
                    ...
                    <div>
                        <h3 className="text-xl font-medium mb-4">Showing {startingProduct} to {endingProduct} of {totalProducts} Products</h3>
                        <ProductCard products={products}></ProductCard>

                        {/* pagination controls */}
                        <div className="mt-6 flex justify-center">
                            <button onClick={() => handlePageChange(currentPage - 1)} disabled={currentPage === 1} className="px-4 py-2 bg-gray-300 text-gray-700 rounded-md mr-2">prev</button>

                            {
                                [...Array(totalPages)].map((_, index) => (
                                    <button onClick={() => handlePageChange(index + 1)} key={index} className={`px-4 py-2 ${currentPage === index + 1 ? 'bg-slate-800 text-white' : 'bg-gray-300 text-gray-800'} rounded-md mx-1`}>{index + 1}</button>
                                ))
                            }

                            <button onClick={() => handlePageChange(currentPage + 1)} disabled={currentPage === totalPages} className="px-4 py-2 bg-gray-300 text-gray-700 rounded-md ml-2">next</button>
                        </div>
                    </div>
                </div>
            </section>
